...
def load_templates(template_dir):
    ...
    for tmpl in ["base", "index", "category"]:
        with open(
            os.path.join(template_dir, f"{tmpl}.html"), "r", encoding="utf-8"
        ) as f:
...
def process_posts(
    content_dir, public_dir, templates, category_map, site_info, nav_html
):
    ...
    for file_path in md_files:
        ...
        if "---" not in raw_text:
            print(
                f"[!] Warning: No '---' separator found in {file_path}. Skipping file."
            )
            continue
        ...
        html_content = markdown.markdown(
            body_part, extensions=["fenced_code", "tables"]
        )

        # Convert Mermaid code blocks into divs that mermaid.js can render
        if 'class="language-mermaid"' in html_content:
            html_content = re.sub(
            ...
            # Inject Mermaid.js script to render the diagram
        ...
        if date:
            post_header += (
                f"<p class='text-red-600 font-bold mb-8 uppercase'>[ DATE: {date} ]</p>"
            )
        if image:
        ...
        categories_data[category].append(
            {"title": title, "link": filename, "image": image, "date": date}
        )
    ...
    for cat_name, posts in categories_data.items():
        ...
        if not posts:
            ...
            for post in posts:
                ...
                img_tag = (
                    f"<img src='{img_src}' class='w-full h-full object-cover grayscale contrast-125 group-hover:grayscale-0 transition-all'>"
                    if img_src
                    else "<div class='w-full h-full bg-black flex items-center justify-center text-red-600 font-bold'>NO_IMG</div>"
                )
        ...
        bg_value = category_map.get(
            cat_name, assets_config.get("default_category_bg", "#000000")
        )
...
def build_whoami(whoami_data, public_dir, templates, site_info, nav_html):
    ...
    skills_html = "".join(
        [
            f"<span class='border-2 border-white px-2 py-1 text-sm m-1 inline-block'>{s}</span>"
            for s in whoami_data.get("skills", [])
        ]
    )
...
def build_index(
    category_html_links, public_dir, templates, site_info, assets_config, nav_html
):
    index_content = templates["index"].replace("{{CATEGORIES}}", category_html_links)

    index_content = index_content.replace(
        "{{PROFILE_PICTURE}}", assets_config.get("profile_picture", "")
    )
    index_content = index_content.replace(
        "{{AUTHOR_NAME}}", site_info.get("author_name", "Author")
    )
    index_content = index_content.replace(
        "{{STATUS_BADGE}}", site_info.get("status_badge", "Active")
    )
    index_content = index_content.replace(
        "{{BIO_DESCRIPTION}}", site_info.get("bio_description", "")
    )
